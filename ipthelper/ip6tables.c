...
static const char optflags[] = { 'n', 's', 'd', 'p', 'j', 'v', 'x', 'i', 'o', '0', 'c' };
...
static void
set_option(unsigned int *options,
	unsigned int option,
	uint8_t *invflg,
	int invert)
{
...
static int
check_entry(const xt_chainlabel chain,
	struct ip6t_entry *fw,
	unsigned int nsaddrs,
	const struct in6_addr *saddrs,
	const struct in6_addr *smasks,
...
int
flush_entries6(const xt_chainlabel chain,
	int verbose,
	struct xtc_handle *handle)
{
	if (!chain)
...
static int
zero_entries(const xt_chainlabel chain,
	int verbose,
	struct xtc_handle *handle)
{
	if (!chain)
...
int
delete_chain6(const xt_chainlabel chain,
	int verbose,
	struct xtc_handle *handle)
{
	if (!chain)
...
static int
list_entries(const xt_chainlabel chain,
	int rulenum,
	int verbose,
	int numeric,
	int expanded,
...
static void print_ip(const char *prefix,
...
static int
list_rules(const xt_chainlabel chain,
	int rulenum,
	int counters,
	struct xtc_handle *handle)
{
