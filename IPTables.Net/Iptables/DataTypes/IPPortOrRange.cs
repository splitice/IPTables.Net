...
namespace IPTables.Net.Iptables.DataTypes
{
    ...
    {
        private static Regex ParsePattern = new Regex(
            @"^(?:(?:(?:\[(?<ip1_1>[0-9a-fA-f\:]+)\]-)?\[(?<ip2_1>[0-9a-fA-f\:]+)\](?::(?<port_1>[0-9\-]+))?)|(?:(?:(?<ip1_2>[0-9\.]+)-)?(?<ip2_2>[0-9\.]+)(?::(?<port_2>[0-9\-]+))?)|(?:(?:(?<ip1_3>[0-9a-fA-f\:]+)-)?(?<ip2_3>[0-9a-fA-f\:]+)))$");
        ...
        public IPAddress UpperAddress => _upperAddress;

        public PortOrRange Port => _port;

        private string PortStringRepresentation()
        {
            if (_port.LowerPort == 0 && _port.UpperPort == 0) return "";
        ...
        public override string ToString()
        {
            ...
            {
                if (strPort.Length == 0) return LowerAddress.ToString();
                return FormatIp(LowerAddress) + ":" + PortStringRepresentation();
            }

            if (strPort.Length == 0) return string.Format("{0}-{1}", FormatIp(LowerAddress), FormatIp(UpperAddress));
        ...
        public static IPPortOrRange Parse(string getNextArg)
        {
            ...
            if (!match.Success) throw new ArgumentException("Invalid IP port or range format");
            ...
            for (var i = 0; i < match.Groups.Count; i++)
            {
                ...
                else if (name == "port_1" || name == "port_2" || name == "port_3") port = g.Value;
            }

            if (lowerIp == null) lowerIp = upperIp;

            if (port == null) return new IPPortOrRange(lowerIp, upperIp);
